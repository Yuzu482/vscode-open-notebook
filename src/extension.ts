...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('on.checkEnv', async () => {
        ...
        const dOk = await checkDocker(); out.appendLine(dOk ? '\u2705 Docker OK' : '\u274C Docker not found');
        const aOk = await checkAPI(baseUrl()); out.appendLine(aOk ? '\u2705 API connected' : '\u26A0 API unreachable');
        if (!aOk && dOk && config.dockerComposePath) { out.appendLine('Starting...'); if (await startDocker(config.dockerComposePath)) { await sleep(5000); out.appendLine(await checkAPI(baseUrl()) ? '\u2705 Started' : '\u274C Timeout'); } }
        updateConn();
    }));

    context.subscriptions.push(vscode.commands.registerCommand('on.switchLang', async () => {
        const p = await vscode.window.showQuickPick(SUPPORTED_LANGS, { title: 'Language' });
        if (p) { await vscode.workspace.getConfiguration('openNotebook').update('language', p.value, true); setLanguage(p.value); tree.refresh(); }
    }));

    context.subscriptions.push(vscode.commands.registerCommand('on.createNotebook', async () => {
        const n = await vscode.window.showInputBox({ title: t('notebook.create.title'), placeHolder: t('notebook.create.placeholder'), validateInput: v => v ? undefined : 'Required' }); if (!n) return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('on.openSource', (s: any) => openDoc('source', s.id)));
    context.subscriptions.push(vscode.commands.registerCommand('on.openNote', (n: any) => openDoc('note', n.id)));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('on.search', async () => {
        ...
        const busy = vscode.window.createQuickPick(); busy.busy = true; busy.show();
        try { const r = await fetch(`${apiUrl()}/search`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: q, search_type: 'text' }) }); const d: any = await r.json(); busy.hide(); const results: any[] = d.results || []; if (!results.length) { vscode.window.showInformationMessage(t('search.noResults')); return; } const items: vscode.QuickPickItem[] = results.map((r2: any) => ({ label: r2.title || r2.name || '(untitled)', detail: (r2.content || '').slice(0, 120) })); const p = await vscode.window.showQuickPick(items, { title: q, placeHolder: `${results.length} results` }); if (p) { out.clear(); out.show(true); out.appendLine(`\u{1F50D} ${q}\n\u{1F4CC} ${p.label}\n\u2500\u2500\n${p.detail || ''}`); } } catch (e: any) { busy.hide(); vscode.window.showErrorMessage(e.message); }
    }));

    // ---- Configure AI (CRITICAL: model_type="language") ----
    ...
    context.subscriptions.push(vscode.commands.registerCommand('on.setDefaultModel', async () => {
        try {
            ...
            const typed = models.filter((m: any) => m.type === 'language');
            if (typed.length) {
                const pick = await vscode.window.showQuickPick(typed.map((m: any) => ({ label: m.name, detail: m.id, id: m.id })), { title: 'Select default chat model', placeHolder: 'Skip' });
                if (pick) { await fetch(`${apiUrl()}/models/defaults`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ default_chat_model: pick.id }) }); }
            }
    ...
    // Import
    context.subscriptions.push(vscode.commands.registerCommand('on.importCurrentFile', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('on.importFile', async (uri: vscode.Uri, uris?: vscode.Uri[]) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('on.importFileFromEditor', async (uri: vscode.Uri) => { vscode.commands.executeCommand('on.importFile', uri, [uri]); }));
}
